      ...
      <div className="mt-6 border-t border-gray-400">
        <dl className="divide-y divide-gray-400">
          <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0 animate-pulse">
            ...
            <dd className="mt-1 leading-6 sm:col-span-2 sm:mt-0 bg-slate-700 dark:bg-slate-400 h-2 text-transparent rounded-full w-fit">
              11148
            </dd>
          </div>
                ...
                <>
                  <Disclosure.Button className="px-4 py-6 sm:flex sm:flex-cols sm:gap-x-4 sm:px-0 w-full">
                    <ChevronUpIcon
                      className={`${
                        open ? "rotate-180 " : ""
                      } h-4 w-4 transition-all duration-300 transform dark:text-white`}
                    />
                    <dt className="leading-6 rounded-full bg-slate-700 dark:bg-slate-400 h-2 text-transparent w-fit animate-pulse">
                      {title} OPR:
                    </dt>
                    <dd className="mt-1 leading-6 sm:col-span-2 sm:mt-0 bg-slate-700 dark:bg-slate-400 h-2 text-transparent rounded-full w-fit animate-pulse">
                      111
                    </dd>
                  </Disclosure.Button>
                  <Disclosure.Panel className="!animate-none divide-y divide-gray-400">
                    {metrics.map(({ id, name }) => (
                      <div
                        key={id}
                        className="ml-4 px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0"
                      >
                        <dt className="leading-6 rounded-full bg-slate-700 dark:bg-slate-400 h-2 text-transparent w-fit animate-pulse">
                          {name}:
                        </dt>
